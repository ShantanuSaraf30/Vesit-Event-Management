...
class MyApp(tk.Tk):
    def __init__(self):
        ...
        button9 = tk.Button(self.sidebar, text="Registraions", command=self.show_frame20)  
        ...
        label=tk.Label(frame1,text="UPCOMING EVENTS",font='times 25 ',fg='black',bg="#8b88e4")
        ...
        for i in myresult:
            h = tk.Label(frame1,font = 'time 17 bold',text =f"{i[1]}",fg="white",bg="navy blue")
            h.pack(side ="top")
        ...
        logo_img = ImageTk.PhotoImage(file="event.png")
        logo_widget = tk.Label(frame14, image=logo_img, bg=frame14.cget("bg"), bd=0)
        logo_widget.image = logo_img
        logo_widget.place(x=0, y=0, relwidth=1, relheight=1)
        ...
        label_counc = tk.Label(frame14, text="Council name",font=('arial',16,'bold'),fg='white',bg='#020546')
        
        name_combo = ttk.Combobox(frame14, values=('Cultural', 'SoRT', 'Music','Sports','ISTE','CSI','IEEE','ISA'))

        
        label_event_roomno = tk.Label(frame14, text="Room no",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_event_roomno = tk.Entry(frame14)
        label_eventname = tk.Label(frame14, text="Event name",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_eventname = tk.Entry(frame14)
        label_date1 = tk.Label(frame14, text="Date",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_date1 = tk.Entry(frame14)
        label_time1 = tk.Label(frame14, text="Time",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_time1 = tk.Entry(frame14)
        label_des1 = tk.Label(frame14, text="Reason",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_des1 = tk.Entry(frame14)
        label_stat1= tk.Label(frame14, text="Status",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_stat1 = tk.Entry(frame14)
        entry_stat1.insert(END,"PENDING")
        floor = tk.Label(frame14, text="Floor",font=('arial',16,'bold'),fg='white',bg='#020546')
        entryfloor = tk.Entry(frame14)
        # Pack the labels and entry fields vertically with padding
        label_counc.pack(pady=10)
        name_combo.pack()
        label_event_roomno.pack(pady=10)
        entry_event_roomno.pack()
        label_eventname.pack(pady=10)
        entry_eventname.pack()
        label_date1.pack(pady=10)
        entry_date1.pack()
        label_time1.pack(pady=10)
        entry_time1.pack()
        label_des1.pack(pady=10)
        entry_des1.pack()
        label_stat1.pack(pady=10)
        entry_stat1.pack()
        floor.pack(pady=10)
        entryfloor.pack()
...
# Define a function to submit the form
        def submitreq():
            counc = name_combo.get()
            roomno = entry_event_roomno.get()
            eventname = entry_eventname.get()
            date = entry_date1.get()
            time = entry_time1.get()
            des = entry_des1.get()
            stat= entry_stat1.get()
            floor=entryfloor.get()
            sql = "insert into rooms values(%s,%s,%s,%s,%s,%s,%s,%s)"
            t = (counc,roomno,eventname,date,time,des,stat,floor)
            mycur.execute(sql,t)
            db.commit()
            success8()
        def success8():
            ...
            h1=tk.Label(succ, text="REQUEST ADDED SUCCESSFULLY", fg="green", font="bold")
            h1.pack()
        ...
        button_submit = tk.Button(frame14, text="Submit",font=('arial',16,'bold'), command=submitreq,fg='white',width=20,bg='navy blue')
        button_submit.pack(pady=20)
        my_button = tk.Button(frame14, text="Graph",command=self.graph)
        my_button.config(width=20, height=3, bg="#f6aeae")
...
# Create the Frame 15 page and add it to the pages dictionary delete event
        ...
        logo_img = ImageTk.PhotoImage(file="event.png")
        logo_widget = tk.Label(frame15, image=logo_img, bg=frame15.cget("bg"), bd=0)
        logo_widget.image = logo_img
        logo_widget.place(x=0, y=0, relwidth=1, relheight=1)
        ...
        label_coun = tk.Label(frame15, text="Council name",font=('arial',16,'bold'),fg='white',bg='#020546')
        
        name_combo = ttk.Combobox(frame15, values=('Cultural', 'SoRT', 'Music','Sports','ISTE','CSI','IEEE','ISA'))
        
        
        label_event_name = tk.Label(frame15, text="Name of the event",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_event_name = tk.Entry(frame15)
        label_time = tk.Label(frame15, text="Time",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_time = tk.Entry(frame15)
        label_date = tk.Label(frame15, text="Date",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_date = tk.Entry(frame15)
        label_ven = tk.Label(frame15, text="Venue",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_ven = tk.Entry(frame15)
        label_des = tk.Label(frame15, text="Event description",font=('arial',16,'bold'),fg='white',bg='#020546')
        entry_des = tk.Entry(frame15)
        label_stat = tk.Label(frame15, text="Status of the event",font=('arial',16,'bold'),fg='white',bg='#020546')
...
# Create the Frame 16 page and add it to the pages dictionary delete event
        ...
        logo_img = ImageTk.PhotoImage(file="event2.png")
        logo_widget = tk.Label(frame16, image=logo_img, bg=frame16.cget("bg"), bd=0)
        logo_widget.image = logo_img
        logo_widget.place(x=0, y=0, relwidth=1, relheight=1)
        ...
        def createbutton(value):
                button = tk.Button(frame16, text=value, command=lambda :button_click(value),fg='white',width=20,bg='navy blue',font=('arial',16,'bold'))
                button.pack(pady=20)     
        for i in myresult:
            createbutton(i[1])
        def button_click(value):
                ...
                frame17_label = tk.Label(frame17, text=value, font=("Arial", 20))
                frame17_label.pack(pady=20)
                self.pages["Frame17"]=frame17
                
               
                mycur1 = db.cursor()
                sql = 'select * from events where ename=%s'
                mycur1.execute(sql,(value,)) 
                ...
                for i in myresult:
                        ...
                        entry_stat = tk.Entry(frame17)
                        entry_stat.insert(END,i[6])
                        button_submit1(frame17)
                label_coun.pack(pady=10)
                entry_coun.pack()
                label_event_name.pack(pady=10)
                entry_event_name.pack()
...
# Define a function to submit the form
        ...
        def home():
            call(["python","Council window.py"])
        # Create a submit button
        def button_submit1(frame17):
                button_submit = tk.Button(frame17, text="Submit",font=('arial',16,'bold'), command=submit_form1,fg='white',width=20,bg='navy blue')
                button_submit.pack(pady=20)

#frame 20 registrations
        frame20 = tk.Frame(self.container, bg="#a2d2ff")
        
        logo_img = ImageTk.PhotoImage(file="event2.png")
        logo_widget = tk.Label(frame20, image=logo_img, bg=frame20.cget("bg"), bd=0)
        logo_widget.image = logo_img
        logo_widget.place(x=0, y=0, relwidth=1, relheight=1)
        
        frame20_label = tk.Label(frame20, text="Registrations", font=("Arial", 20))
        frame20_label.pack(pady=20)
        self.pages["Frame20"] = frame20
        

        label=tk.Label(frame20,text="Events",font='time 30 bold',bg="#a2d2ff")
        label.pack(padx=10,pady=30)
        ...
        mycur.execute(sql) 
        myresult = mycur.fetchall()
        button = tk.Button(frame20, text='Invictus',fg='white',width=20,bg='navy blue',command=self.dis1,font=('arial',16,'bold'))
        button.pack(pady=20)
        
        button = tk.Button(frame20, text='Utsav',fg='white',width=20,bg='navy blue',command=self.dis,font=('arial',16,'bold'))
        button.pack(pady=20)
        
     
        def createbutton(value):
                button = tk.Button(frame20, text=value,fg='white',width=20,bg='navy blue',font=('arial',16,'bold'))
                button.pack(pady=20)
        for i in myresult:
                createbutton(i[1])
    ...
    def show_frame17(self):
        self.show_page("Frame17")
    def show_frame20(self):
        self.show_page("Frame20")
    def chatbot(self):
